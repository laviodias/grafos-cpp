import sys
from main import find_path_with_fuel_limit
...
def run_tests():
    for test_name, test_data in test_cases.items():
        ...
        path, cost, stops = find_path_with_fuel_limit(adj_matrix, start_vertex, mandatory_vertices, fuel_limit)

        if (path,cost,stops) == expected_output:
            print(f"  ✅ Test passed! Expected: {expected_output}, Got: {(path,cost,stops)}")
        else:
            print(f"  ❌ Test failed! Expected: {expected_output}, Got: {(path,cost,stops)}")
